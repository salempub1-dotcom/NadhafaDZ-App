...
export default function NotificationsScreen() {
  ...
  async function loadState(showRefresh = false) {
    if (!user) {
      setChecking(false);
      setRefreshing(false);
      return;
    ...
    try {
      ...
      // Keep the current visual state if the network is temporarily unavailable.
    } finally {
      setChecking(false);
      setRefreshing(false);
    }
  ...
  async function enableNotifications() {
    ...
    try {
      ...
      setEnabled(true);
      Alert.alert('تم تفعيل التنبيهات', `سيصلك إشعار على الهاتف عند تأكيد مرور شاحنة النظافة في ${neighborhoodLabel}.`);
    } catch (e: any) {
            ...
            <View style={styles.cardHead}>
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={[styles.status, !enabled && styles.statusOff]}>
                  {checking ? 'جارٍ التحقق...' : enabled ? 'مفعّلة' : 'غير مفعّلة'}
                </Text>
              </View>
            </View>

            <Text style={styles.body}>
              عند تأكيد مرور الشاحنة سيصلك إشعار على الهاتف. اضغط على الإشعار لفتح الخريطة ومراقبة آخر موقع مؤكد.
            </Text>
